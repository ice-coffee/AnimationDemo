...
public class RoundIndicatorView extends View
{
    ...
    /**
     * 先将画布旋转使起始点位于正上方, 画刻度, 画布旋转一颗度, 再次在正上方处画刻度, 是如此画法
     * @param canvas
     */
    private void drawScale(Canvas canvas) {
        ...
        for (int i = 0; i <= 30; i++) {
            if(i%6 == 0){   //画粗刻度和刻度值
                ...
                //此处画刻度使用负值是由于画布为逆时针旋转,
                canvas.drawLine(0, -radius-sweepInWidth/2,0, -radius+sweepInWidth/2+dp2px(1), paint);

                Log.e("alipay", (-radius-sweepInWidth/2) + ":" + (-radius+sweepInWidth/2+dp2px(1)));
    ...
    /**
     * 绘制文字时, 根据文字绘制原点特性和每次绘制都在原点正上方可知, 当前文字原点的X轴坐标为(-wight/2)
     * @param canvas
     * @param text
     * @param paint
     */
